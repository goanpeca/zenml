...
def compile_child_pipeline(
    ...
    """Compile and persist a snapshot for a dynamic sub-pipeline run.
    ...
    if parent_snapshot.stack is None:
        raise RuntimeError(
            "Cannot compile a sub-pipeline snapshot: parent snapshot has no "
            "associated stack."
        )
    ...
    # The sub-pipeline will use the parent's build, so we simply copy the Docker
