...
def validate_pipeline_outputs(
    ...
    exception = ValueError(
        ...
        "manually store your data as an artifact."
    )
...
def declared_pipeline_output_names(
    pipeline_entrypoint: Callable[..., Any],
) -> List[str]:
    """Output names declared by a dynamic pipeline entrypoint.

    Returns the keys from the entrypoint's return-type annotation in
    deterministic order. Empty if the entrypoint has no annotated returns.

    Args:
        pipeline_entrypoint: The dynamic pipeline entrypoint function.

    Returns:
        Declared output names from the entrypoint annotation.
    """
...
def prepare_pipeline_output_update(
    ...
    output_names = resolve_pipeline_output_names(
        declared_names=declared_pipeline_output_names(pipeline_entrypoint),
        count=len(artifacts),
    )
