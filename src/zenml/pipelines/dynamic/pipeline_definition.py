...
class DynamicPipeline(Pipeline):
    ...
    def submit(
        ...
        **kwargs: Any,
        ...
        if StepContext.get():
            raise RuntimeError(
                "Calling a sub-pipeline is not allowed within a step function."
            )
    ...
    def inline_in_parent_run(self, *args: Any, **kwargs: Any) -> Any:
        """Run a pipeline inline within another dynamic pipeline.

        This method does not create a new pipeline run and instead runs the
        steps as part of the parent pipeline run.
        ...
        if not DynamicPipelineRunContext.get():
            raise RuntimeError(
                "Calling `pipeline.inline_in_parent_run(...)` is only possible "
                "within another dynamic pipeline run."
            )

        if StepContext.get():
            raise RuntimeError(
                "Calling `pipeline.inline_in_parent_run(...)` is not allowed "
